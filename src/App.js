...
const App = () => {
	...
	const [ orderSettings, setOrderSettings ] = useState(JSON.parse((localStorage.getItem('orderSetting') || 'null')) || {});
		...
		<Router>
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						foodAreas={FOOD_AREAS}
						setOrderSettings={({ faster, time, selfService, itemId}) => {
							const nextOrderSettings = {...orderSettings};
							if(!nextOrderSettings[itemId]) {
								nextOrderSettings[itemId] = {};
							}
							nextOrderSettings[itemId].faster = faster;
							nextOrderSettings[itemId].time = time;
							nextOrderSettings[itemId].selfService = selfService;
							setOrderSettings(nextOrderSettings);
						...
						return (
							<Place
								...
								onIncrementPosition={({ id, itemId, areaId }) => {
									...
									console.log(updatedOrder, nextOrderStatuses)
