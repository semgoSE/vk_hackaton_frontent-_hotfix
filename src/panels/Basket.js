...
const Basket = ({ match: { params: { areaId, itemId }}, orderSettings, foodAreas, order, setOrderSettings }) => {
  const [ faster, setFaster ] = useState(orderSettings[itemId] ? orderSettings[itemId].faster : true);
  const [ time, setTime ] = useState(orderSettings[itemId] ? orderSettings[itemId].time: '');
  const [ selfService, setSelfService ] = useState(orderSettings[itemId] ? orderSettings[itemId].selfService: false);
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
          onClick={() => setOrderSettings({ faster, time, selfService, itemId})}
      ...
      <footer className="Place__footer">
        <Link to={`/order/${area.id}/${item.id}`} onClick={() => setOrderSettings({ faster, time, selfService, itemId})} className="Place__order">
          Оплатить {price}
        </Link>
      </footer>
    </div>
